...
import { db } from '../../../db'
import { knowledgeNodes, knowledgeItems, knowledgeMetadata } from '../../../db/schema'
...
class KnowledgeTreeService {
   ...
   */
  private computeItemsHash(items: { content: string }[]): string {
   ...
   */
  private async needsRebuild(items: { content: string }[], domain: Domain): Promise<boolean> {
    if (items.length === 0) return false

    const currentHash = this.computeItemsHash(items)

    // Check stored metadata
    const [meta] = await db
      .select()
      .from(knowledgeMetadata)
      .where(eq(knowledgeMetadata.domain, domain))

    if (!meta) return true // No metadata = first build
    if (meta.itemHash !== currentHash) return true // Data changed

    // Check if tree nodes actually exist
    const stats = await this.getTreeStats(domain)
    if (stats.nodeCount === 0) return true

    return false
  }

  /**
   * Update metadata after tree build
   */
  private async updateMetadata(items: { content: string }[], domain: Domain): Promise<void> {
    const hash = this.computeItemsHash(items)
    const stats = await this.getTreeStats(domain)

    await db
      .insert(knowledgeMetadata)
      .values({
        domain,
        version: 1,
        itemHash: hash,
      ...
      .onConflictDoUpdate({
        target: knowledgeMetadata.domain,
        set: {
          version: 1,
          itemHash: hash,
          lastBuilt: new Date(),
   ...
   * Skips rebuild if items haven't changed (uses hash-based caching).
   */
  async buildTree(
    ...
    const needsBuild = await this.needsRebuild(items, domain)
    if (!needsBuild) {
      console.log(`[KnowledgeTree] Tree for ${domain} is up-to-date, skipping rebuild`)
      return
    }
    ...
    // Update metadata for caching
    await this.updateMetadata(items, domain)
  ...
  async query(queryText: string, domain: Domain, topK = 5): Promise<typeof knowledgeItems.$inferSelect[]> {
    ...
    while (true) {
      ...
      if (children.length === 0) {
        ...
        // Filter by relevance threshold (0.35 minimum - stricter)
        const MIN_RESULT_SCORE = 0.35
        let relevantItems = scoredItems.filter(s => s.score >= MIN_RESULT_SCORE)
        
        // KEYWORD POST-FILTER: Extract keywords from original query and filter
        // Only keep items that contain at least one query keyword
        const queryLower = queryText.toLowerCase()
        const keywords = queryLower
          .replace(/[^a-z0-9\s]/g, '')
          .split(/\s+/)
          .filter(w => w.length >= 3 && !['how', 'much', 'what', 'the', 'have', 'does', 'can', 'are', 'for'].includes(w))
        
        if (keywords.length > 0) {
          const keywordFiltered = relevantItems.filter(s => {
            const contentLower = s.item.content.toLowerCase()
            return keywords.some(kw => contentLower.includes(kw))
          })
          
          // Only use keyword filter if it doesn't eliminate everything
          if (keywordFiltered.length > 0) {
            console.log(`[KnowledgeTree] Keyword filter: ${relevantItems.length} → ${keywordFiltered.length} items (keywords: ${keywords.join(', ')})`)
            relevantItems = keywordFiltered
          }
        }

        console.log(`[KnowledgeTree] Total comparisons: ${comparisons} (vs ${await this.getTotalItemCount(domain)} items)`)
        
        if (relevantItems.length === 0 && scoredItems.length > 0) {
          console.log(`[KnowledgeTree] No results above threshold. Best: ${scoredItems[0].score.toFixed(3)}`)
        }
...
export const knowledgeStoreService = new KnowledgeTreeService()
