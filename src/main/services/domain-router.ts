...
class DomainRouterService {
  ...
  async route(query: string): Promise<RouteResult> {
    ...
    switch (domain) {
      case 'files':
        ...
        results = await routerService.findRelevantFiles(query)
        break
      ...
      case 'personal':
        // Use knowledge tree
        results = await knowledgeTreeService.query(query, domain)
        break
  ...
  async routeMultiple(query: string): Promise<RouteResult[]> {
    ...
    // Always check files
    const fileResults = await routerService.findRelevantFiles(query)
    ...
    for (const domain of ['photos', 'computer', 'personal'] as const) {
      const domainResults = await knowledgeTreeService.query(query, domain)
