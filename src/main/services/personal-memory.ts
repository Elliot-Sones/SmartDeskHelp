...
class PersonalMemoryService {
  ...
  async learn(fact: string, source: 'user' | 'inferred' = 'user'): Promise<void> {
    ...
    await knowledgeTreeService.addItem(
      fact,
      'personal',
  ...
  async recall(query: string): Promise<string[]> {
    const items = await knowledgeTreeService.query(query, 'personal')
  ...
  async initialize(): Promise<void> {
    ...
    const stats = await knowledgeTreeService.getTreeStats('personal')
    ...
    await knowledgeTreeService.buildTree(items, 'personal')
